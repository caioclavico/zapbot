...
(defn- nivel-do-filtro [filtro]
  (when-let [[_ nivel] (re-matches #"(?:(?:nivel|nv)\.?\s*)?([0-9]+)" (normalizar-texto filtro))]
    (js/parseInt nivel 10)))

(defn- filtrar-time [eq filtro]
  (let [filtro (normalizar-texto filtro)
        tipo   (tipo-do-filtro filtro)
        nivel  (nivel-do-filtro filtro)]
    (->> eq
         ...
         (filter (fn [{:keys [registro]}]
                   (let [[pokemon] (treinador/registro->pokemon registro)]
                     (or (str/blank? filtro)
                         (and tipo (some #{tipo} (:tipos pokemon)))
                         (and nivel (= nivel (nivel-pokemon pokemon)))
                         (str/includes? (normalizar-texto (:nome pokemon)) filtro)))))
         vec)))

(defn- descricao-filtro-time [filtro]
  (let [tipo  (tipo-do-filtro filtro)
        nivel (nivel-do-filtro filtro)]
    (cond
      tipo (str "tipo " (get tipos-pt tipo))
      nivel (str "nível " nivel)
      :else (str "nome contendo “" (str/trim filtro) "”"))))
     ...
     (if (and (empty? eq) (empty? em-tratamento))
       ...
       (str (cabecalho) "🧑‍🎓 *Nível de treinador:* " (treinador/nivel-jogador cid pid)
            ...
            (when-not (str/blank? filtro)
              (str "🔎 Filtro: " (descricao-filtro-time filtro) "\n\n"))
            (if (seq filtrado)
              (str/join "\n" (map
            ...
            "\nFiltre com " config/prefix "pokemon time <tipo|nome|nível>.")))))
...
(defn- interpretar-filtro-pokedex [filtro]
  (let [texto (normalizar-texto filtro)
        tipo  (tipo-do-filtro texto)
        nivel (nivel-do-filtro texto)]
    (cond
      (str/blank? texto) nil
      tipo {:tipo tipo :descricao (str "tipo " (get tipos-pt tipo))}
      nivel {:nivel nivel :descricao (str "nível " nivel)}
      :else {:nome texto :descricao (str "nome contendo “" (str/trim filtro) "”")})))
  ...
  (let [cid       (chat-id message)
        ...
        entradas  (->> (vals dex)
                       (filter (fn [entrada]
                                 (let [nome (normalizar-texto (get entrada "nome"))]
                                   (cond
                                     (:tipo filtro) (some #{(:tipo filtro)} (get entrada "tipos"))
                                     (:nivel filtro) (some #(= (:nivel filtro) (:nivel %))
                                                           (get colecao-por-nome nome))
                                     (:nome filtro) (str/includes? nome (:nome filtro))
                                     :else true)))))
    ...
    (p/resolved
     (str (cabecalho) "📚 *Sua Pokédex*\n\n"
          ...
          (when filtro (str "🔎 Filtro: " (:descricao filtro) " — " unicos " espécie(s)\n"))
          (if filtro "Capturas neste filtro: " "Total de capturas: ") capturas "\n🔥 Sequência atual: " sequencia "\n"
          (when (and (:tipo filtro) (some #(empty? (get % "tipos")) (vals dex)))
    ...
    (if (:tipo filtro-pokedex)
      (let [dex (treinador/sincronizar-pokedex-equipe! cid pid)
        ...
        (-> (reduce (fn [anterior lote]
            ...
            (p/then (fn [_] (renderizar-pokedex-pessoal message filtro-pokedex)))))
      ...
      (-> (p/let [texto  (ver-time message filtro)
            ...
            {:medias medias
             ...
             (str (when (pos? ocultos)
                  ...
                  "🔎 Use " config/prefix "pokemon time <tipo|nome|nível> para encontrar o que procura.")})
          (p/catch (fn [err]
    ...
    (cond
      ...
      (let [nivel (treinador/nivel-jogador cid pid)
        ...
        (if (zero? hp-atual)
          ...
          (-> (p/let [selvagem (sortear-selvagem (poder-alvo-caca nivel) (:tipos bioma))
...
(defn- atacar [message indice-texto]
  (let [cid  (chat-id message)
    ...
    (if-let [caca (get @cacadas-selvagens cid)]
      (if (= pid (:pid caca))
        (atacar-selvagem message indice-texto caca)
        (p/resolved (str (cabecalho) "🚫 Essa caçada pertence a outro treinador.")))
      (p/resolved
       (cond
         (nil? jogo)
           ...
           (cond
             ...
             (let [golpe                   (nth (:golpes atacante) indice)
               ...
               (if (zero? (get-in jogo [:hp alvo-marca]))
                 (str (cabecalho) mensagem msg-efeitos (anunciar-vitoria message cid jogo atacante-marca " a batalha"))
                 (if (zero? (get-in jogo [:hp atacante-marca]))
                   (str (cabecalho) mensagem msg-efeitos
                        (anunciar-vitoria message cid jogo alvo-marca (str " - " (:nome atacante) " caiu com o recuo")))
  ...
  (let [cid          (chat-id message)
    ...
    (cond
      ...
      :else (p/resolved (str (cabecalho) "❓ Use " config/prefix "pokemon liga [nome|time <n1,n2,n3>], " config/prefix "pokemon inicial, " config/prefix "pokemon cacar, "
                             config/prefix "pokemon treinador, " config/prefix "pokemon pokedex [número|tipo|nome|nível], " config/prefix "pokemon time [tipo|nome|nível|csv], " config/prefix "pokemon escolher <número>, "
