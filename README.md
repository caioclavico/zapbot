...
### Captura e mensagens de batalha (0.15.5)
...
- Ginásios e ligas continuam com escalações de três Pokémon. O ativo, os favoritos e as escalações existentes são preservados. A migração adiciona o antigo PC ao fim da coleção, sem reordenar os índices atuais, inclusive durante combates, e é aplicada já na leitura do Cassandra.
- Estoque inicial de 26 Pokémon; cada compra em `!pk espaco comprar` acrescenta 50 vagas por **200 moedas**, preço fixo em todas as compras. Expansões já adquiridas continuam valendo. `!pk espaco` mostra a capacidade e as vagas já compradas, incluindo as do antigo PC sem nova cobrança. `!pk pc` apenas orienta a usar a coleção; `!pk pc comprar` continua aceito por compatibilidade. Não há depósito separado.
